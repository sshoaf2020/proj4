...
//according to the crypto documentation
...
app.post("/users", (req, res) => {
  ...
  if (
    ...
    res
      ...
      .redirect(
        "registration.html#" +
          encodeURIComponent("Error, parameter was of an invalid length.")
      );
  }
  //further validate the username and name that they only contain
  //letters, numbers and spaces
  if (
    containsInvalidChars(req.body.username) ||
    containsInvalidChars(req.body.name)
  ) {
    res
      .status(302)
      .redirect(
        "registration.html#" +
          encodeURIComponent("Error, parameter contained invalid characters.")
      );
  }
  ...
  aUser.password = sha256(aUser.password, aUser.date);
  saveUser(aUser);
...
app.post("/auth", (req, res) => {
  ...
  if (getUser(req.body.username) === false) {
    res
      .status(302)
      .redirect(
        "login.html#" + encodeURIComponent("Error, username does not exist.")
      );
  }
  //hash the submitted password using
  //the date stored on the users object
  //and compare the resulting hash to the stored hash
  let aUser = getUser(req.body.username);
  if (
    aUser !== null &&
    aUser.password === sha256(req.body.password, aUser.date)
  ) {
    //if it passes, save the aUser's name into the session
    ...
    res
      .status(302)
      .redirect(
        "login.html#" + encodeURIComponent("Error, password did not match.")
      );
  }
});
